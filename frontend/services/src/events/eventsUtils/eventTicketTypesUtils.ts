...
export type NewEventWithOptionalTicketTypes = EventWithOptionalTicketTypes & {
  price: number;
  capacity: number;
  vacancy: number;
  formId: FormId | null;
...
/** Seeds a General Admission ticket type when creating an event without ticket types. */
export function ensureDefaultEventTicketTypes<T extends NewEventWithOptionalTicketTypes>(data: T): T {
  if (hasEventTicketTypes(data)) {
    return data;
  }
  return {
    ...data,
