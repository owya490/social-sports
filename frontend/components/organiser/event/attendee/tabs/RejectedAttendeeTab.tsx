...
export const RejectedAttendeeActions = ({ order, setSelectedOrderForFormResponses }: RejectedAttendeeActionsProps) => {
  return (
...
export const RejectedAttendeeTab = ({
  ...
  const sortedOrders = Array.from(rejectedOrderTicketsMap.keys()).sort((a: Order, b: Order) =>
  ...
  const renderActions = (item: (typeof tableData)[0]) => {
    const { order } = item;
    return (
      <RejectedAttendeeActions order={order} setSelectedOrderForFormResponses={setSelectedOrderForFormResponses} />
    );
  };
