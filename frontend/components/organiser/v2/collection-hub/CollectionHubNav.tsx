...
import { COLLECTION_HUB_SECTIONS, CollectionHubSection } from "./collectionHubTypes";

type CollectionHubNavProps = {
  current: CollectionHubSection;
  onChange: (section: CollectionHubSection) => void;
};

export function CollectionHubNav({ current, onChange }: CollectionHubNavProps) {
  return (
    <nav className="bg-background" aria-label="Collection sections">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
          role="tablist"
          className="flex gap-1 overflow-x-auto overflow-y-hidden no-scrollbar border-t border-border"
        >
          {COLLECTION_HUB_SECTIONS.map((section) => {
