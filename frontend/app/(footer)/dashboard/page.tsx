...
export default function Dashboard() {
  ...
  // Create dynamic description based on current search/filters
  const getDynamicDescription = () => {
    const { event, location } = getQueryParams();

    if (event && location) {
      return `Find ${event} events in ${location}. Join local sports activities and connect with athletes in your area.`;
    } else if (event) {
      return `Discover ${event} events near you. Join tournaments, leagues and casual games.`;
    } else if (location) {
      return `Find sports events in ${location}. Cricket, basketball, soccer, tennis and more.`;
    } else {
      return "Discover and join sports events in your area. Cricket, basketball, soccer, tennis and more activities near you.";
    }
  };
  ...
  const getDynamicTitle = () => {
    ...
    if (event && location) {
      return `${event} Events in ${location}`;
    } else if (event) {
      return `${event} Events Near You`;
    } else {
  ...
  const getDynamicUrl = () => {
    const { event, location } = getQueryParams();
    let url = "https://www.sportshub.net.au/dashboard";
  ...
  return (
    <>
      <SEO title={getDynamicTitle()} description={getDynamicDescription()} url={getDynamicUrl()} />
      <div>
