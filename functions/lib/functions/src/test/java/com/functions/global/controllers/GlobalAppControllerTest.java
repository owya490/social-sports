...
public class GlobalAppControllerTest {
    ...
    @Test
    public void service_logsCompletionForRoutedStripeWebhook() throws Exception {
        CapturingLogHandler logHandler = new CapturingLogHandler();
        Logger logger = Logger.getLogger(GlobalAppController.class.getName());
        ...
        try {
            ...
            new GlobalAppController((ignoredRequest, response) -> response.setStatusCode(202))
            ...
            assertTrue(logHandler.messages().stream().anyMatch(message ->
                    message.contains("event=http_request_finish")
                            && message.contains("route=stripeWebhook")
                            && message.contains("statusCode=202")));
        } finally {
