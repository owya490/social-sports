...
def send_email_on_create_event(req: https_fn.CallableRequest):
    ...
    try:
        ...
        start_date_string = (
            start_date.ToDatetime().astimezone(SYDNEY_TIMEZONE).strftime("%m/%d/%Y, %H:%M") if start_date else "N/A"
        )
        end_date_string = (
            end_date.ToDatetime().astimezone(SYDNEY_TIMEZONE).strftime("%m/%d/%Y, %H:%M") if end_date else "N/A"
        )
